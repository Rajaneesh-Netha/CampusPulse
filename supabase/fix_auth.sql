-- ============================================================
--  CampusPulse — Rebuild ALL Auth Accounts (idempotent)
--  Run this in Supabase SQL Editor to fix login issues.
--  This properly creates auth.users + auth.identities rows.
-- ============================================================
...
-- ── Step 2: Create accounts with correct identity_data ───────
DO $$
DECLARE
  uid UUID;
BEGIN
  ...
  VALUES (uid, 'Campus Admin', 'admin@campuspulse.edu', 'admin', 'cbit@1979', 'super')
  ON CONFLICT (id) DO UPDATE SET
    name = 'Campus Admin', role = 'admin',
    password_ref = 'cbit@1979', admin_level = 'super';
  ...
  INSERT INTO auth.users (
    ...
    NOW(),
    '{"name":"Demo Student","role":"student","roll_no":"160122733001","password_ref":"Student@123"}'::jsonb,
    '{"provider":"email","providers":["email"]}'::jsonb,
    NOW(), NOW(), '', ''
  );
  ...
  ON CONFLICT (id) DO UPDATE SET
    name='Demo Student', role='student', roll_no='160122733001',
    branch='CSE', year='3rd Year', section='A',
    hostel_block='Block A', hostel_room='301',
    phone='9876543210', password_ref='Student@123';
  ...
  INSERT INTO auth.users (
    ...
    NOW(),
    '{"name":"Demo In-Charge","role":"incharge","password_ref":"Incharge@123"}'::jsonb,
    '{"provider":"email","providers":["email"]}'::jsonb,
    NOW(), NOW(), '', ''
  );
  ...
  ON CONFLICT (id) DO UPDATE SET
    name='Demo In-Charge', role='incharge', employee_id='IC-DEMO-001',
    designation='Block Warden', assigned_block='Block A',
    phone='9876543220', password_ref='Incharge@123',
  ...
  INSERT INTO auth.users (
    ...
    crypt('Student@123', gen_salt('bf')),
    NOW(),
    '{"name":"Rahul Sharma","role":"student","roll_no":"1601-21-748-001","password_ref":"Student@123"}'::jsonb,
    '{"provider":"email","providers":["email"]}'::jsonb,
    NOW(), NOW(), '', ''
  );
  ...
  ON CONFLICT (id) DO UPDATE SET
    name='Rahul Sharma', role='student', roll_no='1601-21-748-001',
    branch='CSE', year='3rd Year', section='A',
    hostel_block='Block A', hostel_room='204',
    phone='9876543211', password_ref='Student@123';
  ...
  INSERT INTO auth.users (
    ...
    crypt('Student@123', gen_salt('bf')),
    NOW(),
    '{"name":"Priya Reddy","role":"student","roll_no":"1601-21-748-002","password_ref":"Student@123"}'::jsonb,
    '{"provider":"email","providers":["email"]}'::jsonb,
    NOW(), NOW(), '', ''
  );
  ...
  ON CONFLICT (id) DO UPDATE SET
    name='Priya Reddy', role='student', roll_no='1601-21-748-002',
    branch='ECE', year='2nd Year', section='B',
    hostel_block='Block C', hostel_room='312',
    phone='9876543212', password_ref='Student@123';
  ...
  INSERT INTO auth.users (
    ...
    crypt('InCharge@123', gen_salt('bf')),
    NOW(),
    '{"name":"Dr. Kumar","role":"incharge","password_ref":"InCharge@123"}'::jsonb,
    '{"provider":"email","providers":["email"]}'::jsonb,
    NOW(), NOW(), '', ''
  );
  ...
  ON CONFLICT (id) DO UPDATE SET
    name='Dr. Kumar', role='incharge', employee_id='IC-2024-003',
    designation='Block In-Charge', assigned_block='Block B',
    phone='9876543221', password_ref='InCharge@123',
    password_reset_required=FALSE;

END $$;


-- ── Step 3: Re-add sample complaints ─────────────────────────
DELETE FROM public.complaints WHERE id IN ('CP-001','CP-002','CP-003','CP-004','CP-005');
...
SELECT 'CP-001', (SELECT id FROM auth.users WHERE email='student@campuspulse.edu'),
       'Water leakage in washroom', 'There is continuous water leakage from the ceiling of the 3rd floor washroom in Block A.',
...
SELECT 'CP-002', (SELECT id FROM auth.users WHERE email='student@campuspulse.edu'),
       'Broken window in common room', 'The window glass in the Block A common room is broken.',
...
SELECT 'CP-004', (SELECT id FROM auth.users WHERE email='priya@campuspulse.edu'),
       'Mess food quality issues', 'The food quality in the mess hall has deteriorated significantly.',
...
-- ════════════════════════════════════════════════════════════
-- DONE! All accounts should now work for login.
-- ════════════════════════════════════════════════════════════
